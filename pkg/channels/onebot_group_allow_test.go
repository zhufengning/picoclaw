...
func TestOneBotHandleMessage_GroupQueueUntilTriggered(t *testing.T) {
	...
	if !strings.Contains(msg.Content, "[Group context before trigger]") {
		t.Fatalf("expected queued context in content, got: %q", msg.Content)
	}
	if !strings.Contains(msg.Content, "今天有点忙") {
		t.Fatalf("expected queued message in content, got: %q", msg.Content)
	}
	if !strings.Contains(msg.Content, "[Triggered message]") || !strings.Contains(msg.Content, "帮我总结一下") {
	...
	if strings.Contains(msg2.Content, "[Group context before trigger]") {
...
func TestOneBotHandleMessage_GroupAutoReplyProbability(t *testing.T) {
	...
	if !strings.Contains(msg.Content, "[Auto-triggered notice]") {
